...
{
	BUILD_BUG_ON(sizeof(struct uffd_msg) != 40);		// 32
	/*
					 ...
					 unsigned long reason)
{
	...
	if (!pte_write(*pte) && (reason & VM_UFFD_WP))
		ret = true;
	if (reason & VM_UFFD_SWAPPING)	// TODO double check this.. | put it in a more optimal spot #^
		ret = true;
	pte_unmap(pte);
...
int dup_userfaultfd(struct vm_area_struct *vma, struct list_head *fcs)
{
	...
	if(octx->mm->def_flags & VM_UFFD_MISSING) {
		if(unlikely(ctx->mm->def_flags & VM_UFFD_MISSING == 0)) {
			ctx->mm->def_flags|=VM_UFFD_MISSING | VM_UFFD_SWAPPING;
		}
...
{	
	...
	if (unlikely(!uffdio_palloc.addr || !ctx->mm)) {
		ctx->released=true;
		goto out;
	}
	...
	case 5 : {
			int ret = uffd_eClrSet(ctx->mm, uffdio_palloc.addr, uffdio_palloc.arg.uaddr);		// ret bas. not needed..
			if (likely(ret == 1)) {
				range.len = 4096;
...
SYSCALL_DEFINE1(userfaultfd, int, flags)
{
	...
	if ((flags & UFFD_USM) !=0) {
		...
		current->mm->def_flags |= VM_UFFD_MISSING | VM_UFFD_SWAPPING/* | VM_PFNMAP*/; /* | VM_IO;  | VM_LOCKED | VM_IO;		 UFFD_USM !  | No MINOR man meh*/
