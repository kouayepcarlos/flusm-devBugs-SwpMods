...
/*void commitSwapNode(struct usm_swap_dev * swapDevice, struct list_head * swapNode) {
    pthread_mutex_lock(&swapDevice->devListLock);
...
int swap_out(struct to_swap * victimNode) {        // promotion / demotion to be done "in line".. i.e. the backfile'd be replaced while copying..all.. the... content.... o_o' TODO further diggin'..
    ...
    */
    ...
    if(usm_clear_and_set(victimNode->proc,victimNode->swapped_address,swap_value(victimNode),0))      // some clearance first..? Of 3 bits...
        return 1;
    //printf("Seeking...\n");
    if(fseeko((FILE *)victimNode->swapDevice->backend, victimNode->snode->offset, SEEK_SET)){            // work this out -> no more backfile per proc., he can just put things differently from the same proc.'s address space.. now that's interesting!   | the backend giving's probably not that stylish... -_-'
#ifdef DEBUG
        perror("[swapDevPols/sp_out/Mayday] Fseeko failed\n");
        getchar();
#endif
        usm_set_pfn(victimNode->proc,victimNode->swapped_address,victimNode->page->physicalAddress, 0);     // drop_swp_out/*_and_free*/...
    ...
    //printf("Writing..\n");
    if (unlikely(fwrite((void*)(victimNode->page->data),SYS_PAGE_SIZE/*ToBeComplxfd*/,1,(FILE *)victimNode->swapDevice->backend)!=1)) {
#ifdef DEBUG
                perror("[swapDevPols/sp_out/Mayday] Swap out writing failed!\n");
                getchar();
#endif
        usm_set_pfn(victimNode->proc,victimNode->swapped_address,victimNode->page->physicalAddress, 0);
        restore_used_page(&((struct optEludeList *)(victimNode->page->usedListPositionPointer))->iulist);
        putSwapNode(victimNode->swapDevice,&victimNode->snode->iulist);
        free(victimNode);
        return 1;
...
int /*generic_*/swap_in(struct to_swap * luckyNode) {        // ..activeProcList | the things about page caching and all.. could create some different thingies than memcpy.... guess we'll be using SPDK anyway.. though o_o' and/or hacking* swp_entry_t
    ...
    if (unlikely(fread((void*)chosenPage->usmPage->data,SYS_PAGE_SIZE,1,(FILE *)luckyNode->swapDevice->backend)!=1)) {
#ifdef DEBUG
    ...
    putSwapNode(luckyNode->swapDevice,&luckyNode->snode->iulist);
    free(luckyNode);
...
void *swap_in_list_and_commit(void * arg) {
    ...
    while(nr>0){
        if(list_empty(&swapList))
            break;
        struct to_swap * luckyNode = list_entry(&swapList,struct to_swap,globlist);
